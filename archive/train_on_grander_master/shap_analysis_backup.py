    ...
    for stance_name, stance_val in stances:
        ...
        for model_name, (model, model_type) in models.items():
            try:
                ...
                # Save raw SHAP values for directional aggregation
                save_raw_shap_values(shap_values, feature_cols, iteration_dir, iteration_num, stance_name, model_name)
...
def save_raw_shap_values(shap_values, feature_names, output_dir, iteration_num, stance_name, model_name):
    """Save raw SHAP values for later directional aggregation"""
    
    # Create a DataFrame with all raw SHAP values
    ...
    raw_output_file = os.path.join(output_dir, f"raw_shap_{stance_name}_{model_name.replace(' ', '_')}.csv")
    
    # Append to existing file or create new one
    if os.path.exists(raw_output_file):
        shap_df.to_csv(raw_output_file, mode='a', header=False, index=False)
    else:
        shap_df.to_csv(raw_output_file, index=False)
