...
for model_class in models:
    ...
    best_params = get_best_params(model_name, best_params_all, dataset=dataset, target=target)
...
# Extract misclassified and correctly classified examples
...
# Log the counts
summary_lines.append(f"🔍 Found {len(misclassified_examples)} examples misclassified by all models.")
summary_lines.append(f"✅ Found {len(correctly_classified_examples)} examples correctly classified by all models.\n")
...
for label_value, label_name in label_mapping.items():
    ...
    # Log the counts
    summary_lines.append(f"📁 Saved {len(misclassified_label_examples)} misclassified examples for label: {label_name}")
    summary_lines.append(f"📁 Saved {len(correctly_classified_label_examples)} correctly classified examples for label: {label_name}")

summary_lines.append("\n📊 All per-label classification files saved successfully!")

df_source_train.to_csv(os.path.join(save_dir, "df_source_train.csv"), index=False)
df_source_test.to_csv(os.path.join(save_dir, "df_source_test.csv"), index=False)
# ✅ Save summary report to text file
summary_file = os.path.join(save_dir, "classification_summary.txt")
