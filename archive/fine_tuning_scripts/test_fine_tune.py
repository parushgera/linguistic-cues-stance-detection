...
# def compute_bert_embeddings_cnn(df, tokenizer, bert_model, max_seq_len=128):
#     """
#     Computes BERT embeddings for CNN models and reshapes them for Conv1D layers.
...
#     Returns:
#         torch.Tensor: CNN-compatible embeddings (batch_size, embedding_dim, sequence_length).
...
#     for text in tqdm(df['text'], desc="Computing BERT Embeddings for CNN"):
#         # Tokenization
...
#             text_embedding = model_output.last_hidden_state.permute(0, 2, 1).squeeze(0)  
#             # Shape after permute: (embedding_dim, sequence_length)
...
#     # Convert to Tensors
#     embeddings = torch.stack(embeddings)  # Shape: (batch_size, embedding_dim, sequence_length)
#     labels = torch.tensor(labels) if labels else None
    
#     print(f"✅ Computed {len(embeddings)} CNN-compatible embeddings")
...
def create_precomputed_loaders_cnn(embeddings, labels, batch_size, shuffle=True):
    """
    Creates DataLoader from precomputed embeddings formatted for CNN models.
    
    Args:
        embeddings (torch.Tensor): Precomputed embeddings (CNN format).
        labels (torch.Tensor or None): Corresponding labels.
        batch_size (int): Batch size.
        shuffle (bool): Whether to shuffle the dataset.

    Returns:
        DataLoader: PyTorch DataLoader formatted for CNN models.
    """
...
def objective(trial, model_class):
    ...
    if model_class in cnn_models:
        ...
        epochs = trial.suggest_int("epochs", 75, 100)
        ...
        epochs = trial.suggest_int("epochs", 5, 20)  # BERT fine-tuning is data-intensive, fewer epochs often work well
        ...
        lr = trial.suggest_float("lr", 1e-6, 5e-4, log=True)  # BERT models typically require lower learning rates
    ...
    if model_class not in bert_models:  
        ...
        train_loader = create_precomputed_loaders_cnn(train_embeddings_cnn, train_labels_cnn, batch_size=batch_size, shuffle=True)
        val_loader = create_precomputed_loaders_cnn(val_embeddings_cnn, val_labels_cnn, batch_size=batch_size, shuffle=False)
        test_loader = create_precomputed_loaders_cnn(test_embeddings_cnn, test_labels_cnn, batch_size=batch_size, shuffle=False) 
    else:
    ...
    if model_class in bert_models:
        ...
        predictions = get_text_predictions(model, test_loader_bert)
...
def run_optuna_for_all_models(models, save_base_dir, target, dataset, n_trials=100, n_jobs=1):
    ...
    for model_class in models:
        ...
        # else:
        #     save_dir = os.path.join(save_base_dir, model_name, dataset )
        save_dir = 'test/test_fine_tuning'
...
dataset = 'covid'
target_key = 'face'
print(dataset, target_key)
if target_key is not None:
    target = target_mappings[dataset][target_key]
else:
    target = None
...
# Compute BERT embeddings specifically for CNN models
    train_embeddings_cnn, train_labels_cnn = compute_bert_embeddings_cnn(df_train, sentence_transformer_tokenizer, sentence_transformer_bert_model)
    val_embeddings_cnn, val_labels_cnn = compute_bert_embeddings_cnn(df_val, sentence_transformer_tokenizer, sentence_transformer_bert_model)
    test_embeddings_cnn, test_labels_cnn = compute_bert_embeddings_cnn(df_test, sentence_transformer_tokenizer, sentence_transformer_bert_model)            
...
completion_file = os.path.join(save_base_dir, "training_completed_successfully.txt")
