...
# Allow direct script invocation regardless of CWD by exposing repo root + scripts/ on sys.path.
_THIS_FILE = Path(__file__).resolve()
for _p in (_THIS_FILE.parent.parent, _THIS_FILE.parent):
...
# sentence_transformer_tokenizer = AutoTokenizer.from_pretrained(TOKENIZER_NAME)
# sentence_tranformer_model = AutoModel.from_pretrained(sentence_transformer_tokenizer)
...
def filter_df_on_target_dataset_split(target=None, dataset=None, split=None, df=None):
    """
    Filters a DataFrame based on the given target, dataset, and/or split type.

    Parameters:
    - target (str, optional): The target stance category to filter on.
    - dataset (str, optional): The dataset name to filter on.
    - split (str, optional): The dataset split type (e.g., "train", "test").
    ...
    if target is not None:
        filter_conditions.append(df['target'] == target)
        print(f"📌 Filtering on Target: {target}")

    if dataset is not None:
        filter_conditions.append(df['dataset'] == dataset)
        print(f"📌 Filtering on Dataset: {dataset}")

    if split is not None:
        filter_conditions.append(df['type'] == split)
        print(f"📌 Filtering on Split: {split}")

    if not filter_conditions:
        print("⚠️ No filters applied! Returning the original DataFrame.")
        return df.copy()  # If no filters are given, return the original dataframe

    # Apply all selected filters
    filtered_df = df.loc[np.logical_and.reduce(filter_conditions)].copy()

    # Drop unnecessary columns if they were filtered on
    drop_columns = []
    if 'type' in df.columns and split is not None:
        drop_columns.append('type')
    if 'dataset' in df.columns and dataset is not None:
    ...
    print(f"✅ Final Filtered DataFrame Size: {filtered_df.shape}")
    return filtered_df
...
def train(model, train_loader, optimizer, device, num_epochs, loss_function, num_classes, val_loader=None, patience=None):
    """
    Trains the model while tracking loss history for plotting.
...
def evaluate_model(model, test_loader, device, loss_function, num_classes, interested_labels, label_names):
    ...
    with torch.no_grad():
        for batch_idx, (text_emb, label) in enumerate(test_loader):  
            ...
            if num_classes == 2:
                predicted = (torch.sigmoid(output) > 0.5).long()  # ✅ Convert logits to binary class labels
            else:
                _, predicted = torch.max(output, 1)
    ...
    print("True", np.unique(all_true_labels))
    print("Predicted",np.unique(all_predictions))
...
# def train_roberta(model, train_dataloader, val_dataloader, learning_rate, epochs, patience=None):
#     """
#     Trains a Transformer model (BERT/RoBERTa) with early stopping and logs loss/accuracy.
...
#     optimizer = Adam(model.parameters(), lr=learning_rate, weight_decay=4e-5)  # ✅ Added weight decay
...
#         # ✅ Wrap train loop in tqdm for cleaner logs
#         train_progress = tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{epochs} - Training", leave=False)
...
#             train_progress.set_postfix(loss=loss.item())  # ✅ Show loss in tqdm progress bar
...
#         with torch.no_grad():
#             for val_input, val_label in val_progress:
#                 attention_mask = val_input['attention_mask'].to(device)
#                 input_ids = val_input['input_ids'].squeeze(1).to(device)
#                 val_label = val_label.to(device)
...
#                 val_progress.set_postfix(loss=loss.item())  # ✅ Show val loss in tqdm progress bar
...
#         else:
#             no_improve_epochs += 1
#             print(f"⚠️ No improvement in validation loss for {no_improve_epochs}/{patience} epochs")
...
def train_bert(model, train_dataloader, val_dataloader, loss_function, learning_rate, epochs, num_classes, patience=None):
    """
    Trains a BERT-based model.
    """
    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda" if use_cuda else "cpu")

    criterion = loss_function  # CrossEntropyLoss for multi-class classification
    optimizer = Adam(model.parameters(), lr=learning_rate, weight_decay=4e-5)  # ✅ Added weight decay
    ...
    for epoch in range(epochs):
        ...
        for input_ids, attention_mask, label in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{epochs} - Training") :
            ...
            if num_classes == 2:
                predictions = (torch.sigmoid(output) > 0.5).long()  # Threshold at 0.5
            else:
        ...
        with torch.no_grad():
            ...
            for input_ids, attention_mask, label in tqdm(val_dataloader, desc=f"Epoch {epoch+1}/{epochs} - Validation"):
                attention_mask = attention_mask.to(device)
                input_ids = input_ids.squeeze(1).to(device)
                ...
                acc = (predictions == val_label).sum().item()
                total_acc_val += acc
        end_time = time.time()
...
class TweetDatasetBertEmbeddings(Dataset):
    ...
    def _get_bert_embeddings(self, sentences):
        ...
        """
        encoded_input = self.tokenizer(sentences, padding="max_length", truncation=True, max_length=self.max_seq_len, return_tensors="pt")

        with torch.no_grad():
            model_output = self.bert_model(**encoded_input)  # Extract BERT embeddings
...
class TweetDatasetTransformer(Dataset):
    def __init__(self, dataframe, tokenizer, max_length=128):
        """
        Dataset class for BERT/RoBERTa embeddings.
        
        Args:
            dataframe (pd.DataFrame): DataFrame containing 'text' and optional 'stance'.
    ...
    def __getitem__(self, idx):
        """
        Tokenizes text and returns input_ids & attention_mask for transformer models.
        """
        text = self.texts[idx]
...
def collate_fn(batch):
    """
    Collate function to dynamically pad sequences for batch processing.

    Args:
        batch (list of tuples): Each tuple contains (input_ids, attention_mask, label).
    
    Returns:
        - input_ids (torch.Tensor): Padded input sequences.
        - attention_mask (torch.Tensor): Attention masks.
        - labels (torch.Tensor, optional): Labels (if available).
    """
...
def create_transformer_loaders(df, batch_size, tokenizer, shuffle=True):
    """
    Creates DataLoader for BERT/RoBERTa-based datasets.

    Args:
        df (pd.DataFrame): Dataframe containing 'text' and optional 'stance'.
    ...
    Returns:
        DataLoader object.
    """
    dataset = TweetDatasetTransformer(df, tokenizer)
...
def get_text_predictions(model, loader, num_classes):
    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda" if use_cuda else "cpu")
    ...
    with torch.no_grad():
        ...
        for input_ids, attention_mask, _ in tqdm(loader):
            ...
            if num_classes == 2:
                ...
                preds = torch.sigmoid(outputs)  # Ensure correct shape
                preds = (preds > 0.5).long()  # Convert to 0 or 1
            else:
                # ✅ Multi-class classification: Use argmax to get class index
                preds = outputs.argmax(dim=1)
