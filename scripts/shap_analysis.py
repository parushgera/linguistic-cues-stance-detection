    ...
    for stance_name, stance_val in stances:
        ...
        for model_name, (model, model_type) in models.items():
            try:
                ...
                # Save raw SHAP values and original feature values for directional aggregation
                save_raw_shap_values(shap_values, feature_cols, iteration_dir, iteration_num, stance_name, model_name, X_test_df, y_test)
...
def save_raw_shap_values(shap_values, feature_names, output_dir, iteration_num, stance_name, model_name, X_test_df, y_test):
    """Save raw SHAP values AND original feature values for feature-outcome analysis"""
    
    # Create a DataFrame with all raw SHAP values (with 'shap_' prefix)
    shap_df = pd.DataFrame(shap_values, columns=[f"shap_{col}" for col in feature_names])
    shap_df['iteration'] = iteration_num
    shap_df['stance'] = stance_name
    shap_df['model'] = model_name
    
    # Add original feature values (with 'feat_' prefix to distinguish from SHAP values)
    feature_df = X_test_df.copy()
    feature_df.columns = [f"feat_{col}" for col in feature_names]
    ...
    # Add target labels
    feature_df['label'] = y_test.reset_index(drop=True)
    
    # Combine SHAP values, feature values, and labels
    combined_df = pd.concat([shap_df, feature_df], axis=1)
    
    # Save combined data with new filename
    raw_output_file = os.path.join(output_dir, f"raw_shap_features_{stance_name}_{model_name.replace(' ', '_')}.csv")
    
    # Also save the old format for backward compatibility
    old_shap_df = pd.DataFrame(shap_values, columns=feature_names)
    old_shap_df['iteration'] = iteration_num
    old_shap_df['stance'] = stance_name
    old_shap_df['model'] = model_name
    old_output_file = os.path.join(output_dir, f"raw_shap_{stance_name}_{model_name.replace(' ', '_')}.csv")
    
    # Save both files
    for df, filename in [(combined_df, raw_output_file), (old_shap_df, old_output_file)]:
...
def analyze_feature_value_relationships(base_dir, output_dir, total_iterations=13):
    """Analyze relationships between feature values and SHAP contributions"""
    
    print(f"\n{'='*60}")
    print("ANALYZING FEATURE VALUE TO SHAP RELATIONSHIPS")
    print(f"{'='*60}")
    
    stances = ["All_Stances", "FAVOR", "AGAINST", "NONE"]
    models = ["Logistic Regression", "XGBoost"]
    
    feature_analysis_dir = os.path.join(output_dir, "feature_value_analysis")
    os.makedirs(feature_analysis_dir, exist_ok=True)
    
    for stance in stances:
        for model_name in models:
            print(f"\nAnalyzing feature-value relationships for {stance} - {model_name}...")
            
            # Collect all feature-SHAP data from iterations
            all_data = []
            
            for i in range(1, total_iterations + 1):
                data_file = os.path.join(base_dir, f"{i:02d}_iteration_shap", 
                                       f"raw_shap_features_{stance}_{model_name.replace(' ', '_')}.csv")
                
                if os.path.exists(data_file):
                    df = pd.read_csv(data_file)
                    df = df[df['iteration'] == i]  # Filter for this iteration
                    all_data.append(df)
            
            if not all_data:
                print(f"  No feature-value data found for {stance}-{model_name}")
                continue
            
            # Combine all iterations
            combined_data = pd.concat(all_data, ignore_index=True)
            
            # Get feature columns (those with 'feat_' prefix)
            feature_cols = [col for col in combined_data.columns if col.startswith('feat_')]
            original_feature_names = [col.replace('feat_', '') for col in feature_cols]
            
            # Analyze each feature
            feature_relationships = []
            
            for i, feature_col in enumerate(feature_cols):
                feature_name = original_feature_names[i]
                shap_col = f"shap_{feature_name}"
                
                if shap_col in combined_data.columns:
                    feature_values = combined_data[feature_col]
                    shap_values = combined_data[shap_col]
                    labels = combined_data['label']
                    
                    # Calculate relationships
                    # When SHAP > 0 (pushes toward correct classification, label=1)
                    positive_shap_mask = shap_values > 0
                    negative_shap_mask = shap_values < 0
                    
                    # When SHAP > 0 (toward correct classification)
                    if positive_shap_mask.sum() > 0:
                        mean_feat_when_shap_positive = feature_values[positive_shap_mask].mean()
                        std_feat_when_shap_positive = feature_values[positive_shap_mask].std()
                        count_shap_positive = positive_shap_mask.sum()
                    else:
                        mean_feat_when_shap_positive = np.nan
                        std_feat_when_shap_positive = np.nan
                        count_shap_positive = 0
                    
                    # When SHAP < 0 (toward misclassification, label=0)
                    if negative_shap_mask.sum() > 0:
                        mean_feat_when_shap_negative = feature_values[negative_shap_mask].mean()
                        std_feat_when_shap_negative = feature_values[negative_shap_mask].std()
                        count_shap_negative = negative_shap_mask.sum()
                    else:
                        mean_feat_when_shap_negative = np.nan
                        std_feat_when_shap_negative = np.nan
                        count_shap_negative = 0
                    
                    # Overall feature statistics
                    overall_mean = feature_values.mean()
                    overall_std = feature_values.std()
                    
                    # Correlation between feature value and SHAP value
                    feature_shap_correlation = feature_values.corr(shap_values)
                    
                    # Store results
                    feature_relationships.append({
                        'feature': feature_name,
                        'mean_feat_when_shap_positive': mean_feat_when_shap_positive,
                        'std_feat_when_shap_positive': std_feat_when_shap_positive,
                        'count_shap_positive': count_shap_positive,
                        'mean_feat_when_shap_negative': mean_feat_when_shap_negative,
                        'std_feat_when_shap_negative': std_feat_when_shap_negative,
                        'count_shap_negative': count_shap_negative,
                        'overall_mean_feat': overall_mean,
                        'overall_std_feat': overall_std,
                        'feature_shap_correlation': feature_shap_correlation,
                        'total_observations': len(feature_values)
                    })
            
            # Save results
            if feature_relationships:
                results_df = pd.DataFrame(feature_relationships)
                output_file = os.path.join(feature_analysis_dir, 
                                         f"feature_value_relationships_{stance}_{model_name.replace(' ', '_')}.csv")
                results_df.to_csv(output_file, index=False)
                print(f"  ✅ Saved feature-value relationships: {output_file}")
                print(f"  📊 Analyzed {len(feature_relationships)} features across {len(combined_data)} observations")
            
    print(f"\n✅ Feature-value relationship analysis complete!")
    return feature_analysis_dir

def aggregate_shap_with_direction(base_dir, output_dir, total_iterations=13, min_occurrences=7):
    """Enhanced aggregation that preserves directional information for summary plots"""
    
    print(f"\n{'='*60}")
    print("AGGREGATING SHAP RESULTS WITH DIRECTIONAL INFORMATION")
    print(f"{'='*60}")
    ...
    for stance in stances:
        for model_name in models:
            ...
            # Collect raw SHAP values from all iterations
            all_raw_shap_data = []
            all_importance_data = []
            
            for i in range(1, total_iterations + 1):
                # Load importance data (existing)
                importance_file = os.path.join(base_dir, f"{i:02d}_iteration_shap", 
                                             f"shap_importance_{stance}_{model_name.replace(' ', '_')}.csv")
                
                # Load raw SHAP data (new)
                raw_file = os.path.join(base_dir, f"{i:02d}_iteration_shap", 
                                       f"raw_shap_{stance}_{model_name.replace(' ', '_')}.csv")
                
                if os.path.exists(importance_file):
                    imp_df = pd.read_csv(importance_file)
                    imp_df['iteration'] = i
                    all_importance_data.append(imp_df)
                
                if os.path.exists(raw_file):
                    raw_df = pd.read_csv(raw_file)
                    raw_df = raw_df[raw_df['iteration'] == i]  # Filter for this iteration
                    all_raw_shap_data.append(raw_df)
            
            if not all_importance_data:
                print(f"  No data found for {stance}-{model_name}")
                continue
            
            # === IMPORTANCE AGGREGATION (existing logic) ===
            combined_importance = pd.concat(all_importance_data, ignore_index=True)
            feature_aggregated = combined_importance.groupby('feature').agg({
            ...
            # === DIRECTIONAL AGGREGATION (new logic) ===
            if all_raw_shap_data:
                print(f"  Processing directional SHAP data...")
                combined_raw = pd.concat(all_raw_shap_data, ignore_index=True)
                
                # Get feature columns (exclude metadata)
                feature_cols = [col for col in combined_raw.columns 
                               if col not in ['iteration', 'stance', 'model']]
                
                # Calculate directional statistics for each feature
                directional_stats = {}
                for feature in feature_cols:
                    if feature in combined_raw.columns:
                        feature_data = combined_raw[feature].dropna()
                        if len(feature_data) > 0:
                            directional_stats[feature] = {
                                'mean_directional': feature_data.mean(),
                                'std_directional': feature_data.std(),
                                'positive_ratio': (feature_data > 0).mean(),
                                'negative_ratio': (feature_data < 0).mean(),
                                'consistent_direction': 'Positive' if feature_data.mean() > 0 else 'Negative',
                                'direction_strength': abs(feature_data.mean()) / (feature_data.std() + 1e-8)
                            }
                
                # Convert to DataFrame and merge with importance data
                directional_df = pd.DataFrame.from_dict(directional_stats, orient='index').reset_index()
                directional_df.rename(columns={'index': 'feature'}, inplace=True)
                
                # Merge directional info with importance aggregation
                feature_aggregated = feature_aggregated.merge(directional_df, on='feature', how='left')
                
                # Create directional consistency plot
                try:
                    plt.figure(figsize=(14, 8))
                    
                    # Get top 20 features by importance
                    plot_data = feature_aggregated.head(20)
                    
                    # Create horizontal bar plot showing directional tendencies
                    y_pos = np.arange(len(plot_data))
                    
                    # Color bars based on consistent direction
                    colors = ['red' if direction == 'Negative' else 'blue' 
                             for direction in plot_data['consistent_direction']]
                    
                    bars = plt.barh(y_pos, plot_data['mean_directional'], color=colors, alpha=0.7)
                    
                    # Add error bars for standard deviation
                    plt.errorbar(plot_data['mean_directional'], y_pos, 
                               xerr=plot_data['std_directional'], 
                               fmt='none', color='black', alpha=0.5, capsize=3)
                    
                    plt.yticks(y_pos, plot_data['feature'])
                    plt.xlabel('Mean Directional SHAP Value')
                    plt.title(f'Feature Direction Consistency - {stance} - {model_name}\n'
                             f'Red = Pushes toward Class 0 (Misclassified), Blue = Pushes toward Class 1 (Correct)')
                    plt.axvline(x=0, color='black', linestyle='-', alpha=0.3)
                    plt.gca().invert_yaxis()
                    
                    # Add text annotations for consistency ratios
                    for i, (idx, row) in enumerate(plot_data.iterrows()):
                        direction_ratio = max(row['positive_ratio'], row['negative_ratio'])
                        plt.text(row['mean_directional'] + row['std_directional'] + 0.01, i, 
                                f'{direction_ratio:.2f}', va='center', fontsize=8)
                    
                    plt.tight_layout()
                    
                    direction_plot_file = os.path.join(final_output_dir, 
                                                     f"direction_consistency_{stance}_{model_name.replace(' ', '_')}.png")
                    plt.savefig(direction_plot_file, dpi=300, bbox_inches='tight')
                    plt.close()
                    
                    print(f"  ✅ Direction consistency plot saved: {direction_plot_file}")
                    
                except Exception as e:
                    print(f"  ⚠️  Could not create direction consistency plot: {e}")
                
                # Create aggregated summary-style plot using directional data
                try:
                    plt.figure(figsize=(12, 8))
                    
                    # Get top features and their directional data
                    top_features = feature_aggregated.head(44)
                    
                    # Create a visualization showing directional patterns
                    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
                    
                    # Left plot: Mean directional values
                    y_pos = np.arange(len(top_features))
                    colors = ['red' if val < 0 else 'blue' for val in top_features['mean_directional']]
                    
                    ax1.barh(y_pos, top_features['mean_directional'], color=colors, alpha=0.7)
                    ax1.errorbar(top_features['mean_directional'], y_pos, 
                               xerr=top_features['std_directional'], 
                               fmt='none', color='black', alpha=0.5, capsize=3)
                    ax1.set_yticks(y_pos)
                    ax1.set_yticklabels(top_features['feature'])
                    ax1.set_xlabel('Mean Directional SHAP Value')
                    ax1.set_title('Average Direction Impact')
                    ax1.axvline(x=0, color='black', linestyle='-', alpha=0.3)
                    ax1.invert_yaxis()
                    
                    # Set appropriate x-axis limits based on data range
                    min_val = (top_features['mean_directional'] - top_features['std_directional']).min()
                    max_val = (top_features['mean_directional'] + top_features['std_directional']).max()
                    
                    # Use a more aggressive scaling approach
                    data_range = max_val - min_val
                    if data_range > 0:
                        # Use 20% padding of the actual data range
                        padding = data_range * 0.2
                        ax1.set_xlim(min_val - padding, max_val + padding)
                    else:
                        # If all values are the same, create a small symmetric range
                        center = top_features['mean_directional'].mean()
                        ax1.set_xlim(center - 0.1, center + 0.1)
                    
                    # Force tight layout around the data
                    ax1.margins(x=0.02)  # Very small margins
                    
                    # Add grid for better readability
                    ax1.grid(True, alpha=0.3, axis='x')
                    
                    # Right plot: Consistency ratios
                    pos_ratios = top_features['positive_ratio'].values
                    neg_ratios = top_features['negative_ratio'].values
                    
                    ax2.barh(y_pos, pos_ratios, color='blue', alpha=0.7, label='Positive (Class 1)')
                    ax2.barh(y_pos, -neg_ratios, color='red', alpha=0.7, label='Negative (Class 0)')
                    ax2.set_yticks(y_pos)
                    ax2.set_yticklabels(top_features['feature'])
                    ax2.set_xlabel('Direction Consistency Ratio')
                    ax2.set_title('Directional Consistency')
                    ax2.axvline(x=0, color='black', linestyle='-', alpha=0.3)
                    ax2.legend()
                    ax2.invert_yaxis()
                    
                    plt.suptitle(f'Aggregated Directional Analysis - {stance} - {model_name}\n'
                               f'Across {total_iterations} iterations')
                    plt.tight_layout()
                    
                    agg_plot_file = os.path.join(final_output_dir, 
                                               f"aggregated_directional_{stance}_{model_name.replace(' ', '_')}.png")
                    plt.savefig(agg_plot_file, dpi=300, bbox_inches='tight')
                    plt.close()
                    
                    print(f"  ✅ Aggregated directional plot saved: {agg_plot_file}")
                    
                except Exception as e:
                    print(f"  ⚠️  Could not create aggregated directional plot: {e}")
            
            # Save enhanced results
            output_file = os.path.join(final_output_dir, f"final_shap_enhanced_{stance}_{model_name.replace(' ', '_')}.csv")
            feature_aggregated.to_csv(output_file, index=False)
            
            print(f"  ✅ Enhanced results saved to {output_file}")
            print(f"  📊 Features with ≥{min_occurrences} occurrences: {len(feature_aggregated)}")
...
def compare_shap_with_existing_linear_results(shap_dir, linear_base_dir, output_dir):
    ...
    for stance in stances:
        for model_file_name, model_display_name in models:
            ...
            # Load SHAP results
            shap_file = os.path.join(shap_dir, f"final_shap_enhanced_{stance}_{model_file_name}.csv")
...
def run_complete_shap_analysis(base_dir, output_dir, linear_iterations_dir, total_iterations=13):
    ...
    # Step 4: Aggregate results across iterations (with directional analysis)
    print(f"\n{'='*60}")
    print("STEP 4: AGGREGATING SHAP RESULTS ACROSS ITERATIONS")
    print(f"{'='*60}")
    # aggregate_shap_across_iterations(output_dir, output_dir, total_iterations)  # Comment out or remove
    
    # Step 4b: Enhanced directional aggregation (does everything the original does + more)
    print(f"\n{'='*60}")
    print("STEP 4B: ENHANCED DIRECTIONAL AGGREGATION")
    print(f"{'='*60}")
    aggregate_shap_with_direction(output_dir, output_dir, total_iterations)
    
    # Step 4c: Analyze feature value to SHAP relationships
    print(f"\n{'='*60}")
    print("STEP 4C: FEATURE VALUE TO SHAP RELATIONSHIP ANALYSIS")
    print(f"{'='*60}")
    analyze_feature_value_relationships(output_dir, output_dir, total_iterations)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run SHAP analysis and/or comparison vs LR coefficients.")
    parser.add_argument("--mode", choices=["full", "compare"], default="full",
                        help="full: run 13-iteration SHAP + aggregation + comparison. "
                             "compare: only re-run the comparison against existing SHAP outputs.")
    args = parser.parse_args()

    base_dir = RESULTS_DIR / "evaluation_results"
    output_dir = RESULTS_DIR / "comprehensive_shap_analysis"
    linear_iterations_dir = RESULTS_DIR / "iterations"

    if args.mode == "full":
        run_complete_shap_analysis(str(base_dir), str(output_dir), str(linear_iterations_dir))
    else:
        shap_dir = output_dir / "final_shap_aggregated"
        run_comparison_only(str(shap_dir), str(linear_iterations_dir), str(output_dir))