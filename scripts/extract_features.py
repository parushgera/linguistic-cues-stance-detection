...
LEXICON_PATH = REPO_ROOT / "lexicons" / "subjclueslen1-HLTEMNLP05.tff"
...
def process_and_save_features(file):
    ...
    df_features = df.copy()
    df_features[feature_columns] = df['text'].apply(extract_all_features)
...
if __name__ == "__main__":
    import sys
    # Default behaviour: extract features for the two test splits used in the paper.
    targets = sys.argv[1:] if len(sys.argv) > 1 else [
        str(PROCESSED_DIR / "except_wtwt_test.csv"),
        str(PROCESSED_DIR / "wtwt_test.csv"),
    ]
    for path in targets:
        if os.path.exists(path):
            process_and_save_features(path)
        else:
