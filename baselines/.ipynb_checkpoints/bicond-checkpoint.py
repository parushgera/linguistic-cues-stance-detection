...
class BiCond(nn.Module):
    def __init__(self, num_classes):
        ...
        self.hidden = 512  # Hidden size of LSTM
        self.input_size = 384  # BERT embedding dimension
        self.dropout = nn.Dropout(0.2)

        # BiLSTM for encoding text
        self.text_bilstm = nn.LSTM(self.input_size, self.hidden, bidirectional=True, batch_first=True, dropout=0.2)
    ...
    def forward(self, text_emb):
        ...
        # Encode the document using BiLSTM
        _, (txt_last_hn, _) = self.text_bilstm(x)  # (2, batch, hidden)

        # Concatenate forward and backward hidden states
        txt_last_hn = torch.cat((txt_last_hn[0], txt_last_hn[1]), dim=1)  # (batch, 2*hidden)
